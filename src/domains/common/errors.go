...
const (
	isNullOrEmptyText     string = "%s is null or empty !"
	invalidUserIdText     string = "The User '%s' is invalid!"
	invalidMessageText    string = "Message '%s' is invalid!"
	invalidConnectionText string = "Invalid Connection!"
	connectionClosedText  string = "Connection Closed!"
...
func InvalidParamsError(errorString string) *ApplicationError {
	...
	if errorString == "unexpected end of JSON input" {
		return &ApplicationError{
			Msg:              "Error, empty body data:",
			Code:             CodeErrors["INVALID_PARAMS"],
			HttpError:        http.StatusForbidden,
			ErrorDescription: "empty data",
		}
	}

	for _, itemError := range splitStringError {
		splitItemString := strings.Split(itemError, "Error:")
		if len(splitItemString) > 1 {
